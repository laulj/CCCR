...
def run_paddle_ocr_as_api(
    source=0, flip=False, use_popup=False, skip_first_frames=0, text_sys=None, args=[]
):
    """
        ...
        while True:
            ...
            for id, (text, score) in enumerate(rec_res):
                ...
                if valid == "PASSED":
                    ...
                    results_log_lines.append(
                        frame_time.strftime("%Y-%m-%dT%H-%M-%S")
                        + "\t"
                        + "{}\t{:.3f}".format(temp_text, temp_score)
                        + "\n"
                    )
                ...
                for id, res in enumerate(
                    results_log_lines[results_log_lines_skip_to_index:],
                    results_log_lines_skip_to_index,
                ):
                    ...
                    for id_next, res_next in enumerate(
                        results_log_lines[id + 1 :], id + 1
                    ):
            ...
            if args.save_as_image and saveImage and len(valid_dt_boxes) != 0:
                ...
                cv2.imwrite(
                    os.path.join(args.draw_img_save_dir, image_path),
                    draw_img[:, :, ::-1],
                )
            ...
            cv2.putText(
                img=draw_img,
                text=f"{frame_time.strftime('%Y-%m-%d %H:%M:%S')} Inference time: {processing_time_det:.1f}ms ({fps:.1f} FPS)",
                org=(20, 30),
            ...
            if use_popup:
                ...
                _, encoded_img = cv2.imencode(
                    ext=".jpg", img=draw_img, params=[cv2.IMWRITE_JPEG_QUALITY, 100]
                )
...
def run_paddle_ocr(
    ...
    try:
        ...
        player.start()
        if use_popup:
            title = "Character Container Code Inferencing... Press ESC to Exit"
            cv2.namedWindow(
                winname=title, flags=cv2.WINDOW_GUI_NORMAL | cv2.WINDOW_AUTOSIZE
            )
        ...
        while True:
            ...
            """
            CCC scenarios (order matters):
            1. Multi-row two-lines valid detection,
            2. Multi-row first line valid and second invalid,
            3. Multi-row first line invalid and second valid,
            4. Multi-row invalid line and single-row valid,
            5. Single-row invalid, multirow valid,
            6. Single-row valid, invalid multi-row,
            7. Single-row valid, valid multi-row,

            Checking steps:
            1. Check if the CCC is valid, if yes, skip to 7,
            2. Check if the previous CCC is Multi-row, if no, skip to 5,
            3. Concatenate the previous and current CCC,
            4. Check if the concatenated CCC is valid,
            5. Check if the CCC is Multi-row, i.e. length <= 4.
            6. Append the result temporarily,
            7. Append the CCC as valid result.
            """
            valid_rec_res = []
            valid_dt_boxes = []
            _predict = [("NULL", 0)]
            ...
            for id, (text, score) in enumerate(rec_res):
                ...
                # 1. Check if the CCC is valid,
                valid = ccc_validate(temp_text)

                if valid != "PASSED":
                    # 2. Check if the previous CCC is Multi-row,
                    if _predict[0][0] != "NULL":
                        # 3. Concatenate the previous and current CCC,
                        temp_text = _predict[0][0] + " " + text
                        temp_score = (score + _predict[0][1]) / 2
        ...
        for idx, image_file in enumerate(image_file_list):
            ...
            for index, img in enumerate(imgs):
                ...
                for id, (text, score) in enumerate(rec_res):
                    ...
                    # 1. Check if the CCC is valid,
                    valid = ccc_validate(text)

                    if valid != "PASSED":
                        # 2. Check if the previous CCC is Multi-row,
                        if _predict[0][0] != "NULL":
                            # 3. Concatenate the previous and current CCC,
                            text = _predict[0][0] + " " + text
                            score = (score + _predict[0][1]) / 2
                        ...
                        # 7. Append the CCC as valid result.
                        valid_rec_res.append((text, score))
                        valid_dt_boxes.append(dt_boxes[id])
                        '''print(
                            "append:",
                ...
                if len(valid_dt_boxes) != 0:
                    res = [
                        {
                            "transcription": valid_rec_res[i][0],
                            "points": np.array(valid_dt_boxes[i])
                            .astype(np.int32)
                            .tolist(),
                        }
                        for i in range(len(valid_dt_boxes))
                ...
                if args.save_as_image:
                    ...
                    boxes = valid_dt_boxes
                    txts = [valid_rec_res[i][0] for i in range(len(valid_rec_res))]
                    scores = [valid_rec_res[i][1] for i in range(len(valid_rec_res))]
                    ...
                    if len(valid_dt_boxes) != 0:
                        image_path = (
                            now.strftime("%Y-%m-%dT%H-%M-%S")
                            + "_"
                            + "{}_{}".format(txts[0], math.ceil(scores[0] * 100))
                        )
                    else:
                        image_path = (
                            now.strftime("%Y-%m-%dT%H-%M-%S")
                    ...
                    save_file = (
                        image_path
                        + "_"
                        + os.path.basename(save_file).split(".")[0]
                        + "."
                        + os.path.basename(save_file).split(".")[1]
                    )

                    cv2.imwrite(
                        os.path.join(
                            args.draw_img_save_dir, os.path.basename(save_file)
                        ),
                        draw_img[:, :, ::-1],
                    )
                    logger.debug(
            ...
            if imgCount == len(image_file_list):
                with open(args.save_log_path + "result.txt", "a") as fout:
                    fout.writelines(results_log_lines)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if int(thread_id) == 0 and args.test_run == True:
